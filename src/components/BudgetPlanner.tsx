...
import { Check, ArrowRight, Building2, DollarSign, Target } from 'lucide-react';
...
const budgetTiers = [
  {
    ...
    studio: '1 Shoot Session (8 Cinematic Video Reels)',
  ...
  },
  {
    ...
    studio: '2 Shoot Sessions (16 Cinematic Video Reels)',
      ...
      <div className="max-w-6xl mx-auto px-6 lg:px-12 text-center">
        ...
        <div className="grid lg:grid-cols-12 gap-8 text-left">
          ...
          <div className="lg:col-span-6 space-y-6">
            ...
            {/* Step 1: Industry */}
            <div className="sociallyin-card p-6 space-y-3 bg-[#F4F8FE] border border-[#C2DBFE]">
              ...
              <div className="grid sm:grid-cols-2 gap-2.5">
                {verticals.map((vert, idx) => (
                  <button
                    ...
                    className={`p-3 rounded-xl text-left text-xs font-bold font-sans transition-all border ${
                      selectedVertical === vert
                        ? 'bg-sociallyin-blue text-white border-sociallyin-blue shadow-md'
                        : 'bg-white text-slate-800 border-gray-200 hover:border-sociallyin-blue'
                    }`}
                  >
                    {vert}
                  </button>
                ...
                {budgetTiers.map((tier) => (
                  <button
                    ...
                    className={`w-full p-4 rounded-xl text-left transition-all border flex items-center justify-between ${
                      selectedTier.id === tier.id
                        ? 'bg-blue-50/90 border-sociallyin-blue shadow-md'
                  ...
                  >
                    ...
                    <div className="text-right">
                      <span className="font-gothic text-2xl text-sociallyin-blue font-bold">{tier.usd}</span>
                    </div>
                  </button>
                ))}
              </div>
          ...
          <div className="lg:col-span-6">
            <div className="sociallyin-card p-8 bg-white border-2 border-sociallyin-blue shadow-2xl space-y-6 sticky top-24">
              ...
              <div className="border-b border-gray-200 pb-4 space-y-1">
                ...
                <p className="text-xs font-bold text-slate-600 font-sans uppercase">
                  Target Industry: <strong className="text-sociallyin-blue">{selectedVertical}</strong>
                </p>
              </div>

              <div className="space-y-3.5 text-xs font-sans text-slate-800 font-medium">
                ...
                <div className="p-3.5 rounded-xl bg-[#F4F8FE] border border-[#C2DBFE] space-y-1">
                  ...
                  <p className="font-bold text-slate-900">{selectedTier.website}</p>
                </div>

                <div className="p-3.5 rounded-xl bg-[#F4F8FE] border border-[#C2DBFE] space-y-1">
                  <span className="font-gothic text-lg text-sociallyin-blue uppercase block">VIDEO CREATIVES & REELS PRODUCTION</span>
                  <p className="font-bold text-slate-900">{selectedTier.studio}</p>
                </div>

                <div className="p-3.5 rounded-xl bg-[#F4F8FE] border border-[#C2DBFE] space-y-1">
                  <span className="font-gothic text-lg text-sociallyin-blue uppercase block">SOCIAL MEDIA HANDLING & PAID CAMPAIGNS</span>
                  <p className="font-bold text-slate-900">{selectedTier.social}</p>
                </div>

                <div className="p-3.5 rounded-xl bg-[#F4F8FE] border border-[#C2DBFE] space-y-1">
                  <span className="font-gothic text-lg text-sociallyin-blue uppercase block">NOTION DATABASE API & WHATSAPP LEAD CAPTURE</span>
                  <p className="font-bold text-slate-900">{selectedTier.notion}</p>
                </div>
              ...
              <div className="p-4 rounded-xl bg-amber-50 border border-amber-200 text-amber-900 space-y-1">
                ...
                <p className="text-xs font-semibold font-sans leading-relaxed">
